...
data_silage_maize = pd.read_csv(r'C:\Users\asalr\OneDrive\Escritorio\thesis\final DATA\winter barley\data_winter_barley.csv', sep=',')
...
data_after2002 = data_silage_maize.loc[data_silage_maize['date'].dt.year > 2001]
Filtered = data_after2002.loc[data_after2002['date'].dt.month == 6]
Filtered = Filtered.loc[Filtered['date'].dt.day <= 2]
...
dropped = data_after2002.drop(columns=['date', 'NUTS_CODE', 'crop', 'year', 'yield'])
dropped.columns
#x = np.zeros((285,12,8))
data_np = dropped.values
data_3d = data_np.reshape(2466,8,4)
data_3d[[20],[5],[1]]
len(data_3d)


converted_2D = data_3d.reshape(-1, 32)
len(converted_2D)

clmns = ['NDVI_May_1', 'NDWI_May_1', 'ET_May_1', 'LST_May_1', 'NDVI_May_2', 'NDWI_May_2', 'ET_May_2', 'LST_May_2', 
         'NDVI_May_3', 'NDWI_May_3', 'ET_May_3', 'LST_May_3', 'NDVI_May_4', 'NDWI_May_4', 'ET_May_4', 'LST_May_4', 
         'NDVI_June_1', 'NDWI_June_1', 'ET_June_1', 'LST_June_1', 'NDVI_June_2', 'NDWI_June_2', 'ET_June_2', 'LST_June_2', 
         'NDVI_June_3', 'NDWI_June_3', 'ET_June_3', 'LST_June_3', 'NDVI_June_4', 'NDWI_June_4', 'ET_June_4', 'LST_June_4']
...
# nan_rows_valueCounts.index.names = ['Index'] # for pandas version 2
# nan_rows_valueCounts.rename(columns={'count': 'NUTS_CODE'}, inplace=True) # for pandas version 2
...
xgb_0 = xgb.XGBRegressor(random_state = 66)
...
xgb_1 = xgb.XGBRegressor(objective='reg:squarederror', random_state = 66)
...
Default_model = evaluate(xgb_1, X_test, y_test)
...
Default_model_n_trees = xgb_1.get_params()['max_depth']
Default_model_n_features = xgb_1.get_params()['learning_rate']
...
reg_alpha = [0, 0.001, 0.005, 0.01, 0.05, 0.1, 0.5, 1, 2, 3]
# Method of selecting samples for training each tree
reg_lambda = [0.01, 0.1, 1, 2, 3, 4, 5]
...
xgb_random = RandomizedSearchCV(estimator = xgb_2, param_distributions = random_grid, scoring = 'neg_root_mean_squared_error', 
                               n_iter = 200, cv = 5, verbose=2, random_state=66, n_jobs = -1)
...
randomSearch_n_trees = xgb_random.best_params_['max_depth']
randomSearch_n_features = xgb_random.best_params_['learning_rate']
...
# =============================================================================
# param_grid = {'n_estimators': [700, 800, 1000, 1100],
#               'max_depth': [None, 5, 6],
#               'gamma': [0.05, 0.1, 0.2],
...
#               }
# =============================================================================
param_grid = {'n_estimators': [500, 700, 800, 900],
...
xgb_3 = xgb.XGBRegressor(objective='reg:squarederror')
# Instantiate the grid search model
grid_search = GridSearchCV(estimator = xgb_3, param_grid = param_grid, scoring = 'neg_root_mean_squared_error',
...
gridSearch_n_trees = grid_search.best_params_['max_depth']
gridSearch_n_features = grid_search.best_params_['learning_rate']

#******************************************************************************************************************
#******************************************************************************************************************

comparison = {'Model': ['Default Model', 'Randomized Search Model', 'Grid Search Model'],
...
plt.xlabel(''); plt.ylabel('Accuracy (%)'); plt.title('Accuracy Comparison', fontsize=12);
...
plt.xlabel(''); plt.ylabel('Error (ton)'); plt.title('Error Comparison', fontsize=12);
...
plt.xlabel('Model'); plt.ylabel('R squared'); plt.title('R squared Comparison', fontsize=12);
...
plt.xticks(xvalues, comparison['Model'], rotation = 45, fontsize = 12)
plt.ylim(ymin = 0.0, ymax = 4.0)
plt.xlabel('Model'); plt.ylabel('RMSE (ton)'); plt.title('RMSE Comparison', fontsize=12);

plt.subplots_adjust(wspace=0.30, hspace=0.30)
plt.savefig(r'C:\Users\asalr\OneDrive\Escritorio\winter barley\rr.jpg', bbox_inches = 'tight', dpi=600)
plt.show()
...
plt.ylabel('Error (ton)', fontsize=14)    # Adjusting y-axis label font size
plt.title('2016', fontsize=16)
sns.violinplot(y=aq['error'], inner=None)
sns.stripplot(y=aq['error'], color="black", edgecolor="gray")
#sns.boxplot(y=grid_accuracy[3])
plt.savefig(r'C:\Users\asalr\OneDrive\Escritorio\winter barley\rrfr.jpg',  bbox_inches = 'tight', dpi=600)

##############################################################################

result_2019 = pd.DataFrame({'Prediction': y_pred_3, 'Actual': y_test.values}, index= lbl.inverse_transform(test['NUTS_CODE']))
result_2019.to_csv(r'C:\Users\asalr\OneDrive\Escritorio\winter barley\out2019.csv')  
result_2018 = pd.DataFrame({'Prediction': y_pred_1, 'Actual': y_test.values}, index= lbl.inverse_transform(test['NUTS_CODE']))
result_2018.to_csv(r'C:\Users\asalr\OneDrive\Escritorio\winter barley\out2018.csv')  
result_2017 = pd.DataFrame({'Prediction': y_pred_1, 'Actual': y_test.values}, index= lbl.inverse_transform(test['NUTS_CODE']))
result_2017.to_csv(r'C:\Users\asalr\OneDrive\Escritorio\winter barley\out2017.csv')  
result_2016 = pd.DataFrame({'Prediction': y_pred_1, 'Actual': y_test.values}, index= lbl.inverse_transform(test['NUTS_CODE']))
result_2016.to_csv(r'C:\Users\asalr\OneDrive\Escritorio\winter barley\out2016.csv')  
result_2015 = pd.DataFrame({'Prediction': y_pred_1, 'Actual': y_test.values}, index= lbl.inverse_transform(test['NUTS_CODE']))
result_2015.to_csv(r'C:\Users\asalr\OneDrive\Escritorio\winter barley\out2015.csv')  

fig, ax = plt.subplots(4, 2, figsize=(20,18), gridspec_kw={'width_ratios': [2.5, 1.5]})
...
ax[3, 0].set_xlabel('Counties Codes') # if we need more plots, we should delete this line.
...
fig.savefig(r'C:\Users\asalr\OneDrive\Escritorio\winter barley\ttfr.jpg', dpi=600)
...
features = [('NDWI_June_2', 'LST_May_1')]
plt.figure(figsize=(10, 8))
PartialDependenceDisplay.from_estimator(best_grid, X_train, features)
plt.xlabel('NDWI_June_2', fontsize=14)    # Adjusting x-axis label font size
plt.ylabel('LST_May_1', fontsize=14)    # Adjusting y-axis label font size
plt.title('PDP 2016', fontsize=16)            # Adjusting title font size
...
plt.savefig(r'C:\Users\asalr\OneDrive\Escritorio\winter barley\partial.png', dpi=600)
plt.show()
